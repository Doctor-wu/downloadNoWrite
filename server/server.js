        ...
        if (response.data && response.data.state === 200) {
          fs.writeFile(
              ...
              (err) => {
                ...
                emitMsg({
                  message: "参数获取成功,流程已启动",
                });
                clearDot(response.data && response.data.variables.proInstId, token);
                res.end();
...
let judgeOutSchool = async function () {
  ...
  return axios
      .get(
          `http://e.dgut.edu.cn/api/home/getPendingTask?search=&offset=0&length=${count}`,
          {
            headers: {
              "X-Authorization-access_token": token,
            },
      ...
      .then((res) => {
        console.log(
            `// =============================================总共还剩${res.data.info.total}人=================================================== //`
        );
        if (res.data.info.total == 0) {
          throw new Error("审批完毕!");
        }
        emitMsg(
            `正在审批: ${JSON.stringify(res.data.info.list.map((s) => s.creator))}`
        );
      ...
      .then((list) => {
        return list.map((item) => {
          return axios
              .get(
                  `http://e.dgut.edu.cn/ibps/business/v3/bpm/task/getFormData?taskId=${item.task_id}`,
                  {
                    headers: {
                      "X-Authorization-access_token": token,
                    },
              ...
              .then((stuInfo) => {
                ...
                return {
                  ...
                  boData: stuInfo.data.data.boData,
                };
              })
              .catch((e) => {
      ...
      .then((infoArr) => {
        return infoArr.map((pStu) =>
            pStu
                .then((stu) => {
                  return axios(
                      `http://e.dgut.edu.cn/ibps/business/v3/bpm/task/agree`,
                      {
                        method: "POST",
                        headers: {
                          "X-Authorization-access_token": token,
                      ...
                      .then((res) => {
                        if (res.data.state === 200) {
                          emitMsg(
                              `<h3 style="color: red">${stu.creator}审批成功!</h3></br>`
                          );
                          console.log(`${stu.creator}审批成功!`);
                          success++;
                        } else {
                          emitMsg(
                              `${stu.creator}出现特殊情况,${JSON.stringify(res.data)}`
                          );
                        ...
                        emitMsg(
                            `成功<h3 style="color: red;display: inline-block">${success}</h3>人，失败${failed}人`
                        );
                      })
                      .catch((e) => {
